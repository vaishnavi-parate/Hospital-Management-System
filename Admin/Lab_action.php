...
{
$id=$_POST['id'];
$delete = "DELETE FROM lab_registration WHERE LID='$id'";
mysql_query( $delete);
...
{
$id=$_POST['ide'];
mysql_query("UPDATE lab_registration SET Apprve = 'YES' WHERE LID=".$id."");
...
<h4 class="margin-bottom-15">All Labs</h4>
...
$dval=$_SESSION['Duserid'];
					...
					$per_page = 10; 
					$select_table = "select * from lab_registration where concat(name,' ',Lab_name,' ',contact) like '%".$Cdateval."%'";
...
$select_table =$select_table." order by LID DESC limit $start,$per_page";
$fetch= mysql_query($select_table);
}
...
{
?>
<a href="#" class="Edit" id="<?php echo $row["LID"]; ?>"><button class="btn btn-sm btn-primary">[ Approve ]</button></a>
<?php
}
?>                                                          
<a href="#" class="ABCD" id="<?php echo $row["LID"]; ?>"><button class="btn btn-sm btn-danger">
                                                          [ Delete ]</button></a>
														  </TD> 													  </TD> 
</TR>
